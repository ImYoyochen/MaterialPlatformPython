...
# Chargez les indicateurs d'éléments
    element_indicators = pd.read_csv(open_url('gen_element_imputed.csv'), sep=',')
...
output_div.innerText = app.layout

# 将布局渲染为字符串
# layout_html = str(app.layout)

# # 将 Dash 应用的 HTML 部分保存到一个单独的 HTML 文件
...
#     debug = False if os.environ.get('PORT') else False  # Définissez debug sur False si PORT est défini (c'est-à-dire sur Heroku)
